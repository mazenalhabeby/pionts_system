...
import {
  SettingsIcon, LogOutIcon, SidebarToggleIcon,
  MonitorIcon, SunIcon, MoonIcon, BookIcon, CreditCardIcon, BuildingIcon,
} from '@pionts/shared';
import {
  DropdownMenu,
  DropdownMenuContent,
...
import { PRIMARY_ICON_MAP, SECONDARY_ICON_MAP, SIDEBAR_OPEN_W } from './constants';
...
export default function Sidebar({ collapsed, setOpen, setMobileOpen, visibleNavItems, visibleSecondaryNav }: SidebarProps) {
  ...
  return (
    <div className="flex flex-col h-full" style={{ width: SIDEBAR_OPEN_W }}>
          ...
          <div className="flex items-center gap-2 px-1 min-w-0 flex-1">
            ...
            {collapsed ? (
              <button
                ...
                title="Open sidebar"
              >
                <span className="bg-linear-to-br from-accent to-[#ff6a00] text-white font-extrabold text-[13px] tracking-wide rounded-md leading-none flex items-center justify-center w-[30px] h-[28px]">
      ...
      <nav className="flex-1 overflow-y-auto px-2 pt-3">
        <div className={`text-[10px] font-semibold uppercase tracking-wider text-text-faint px-3 mb-1.5 whitespace-nowrap transition-opacity duration-200 ${collapsed ? 'opacity-0' : 'opacity-100'}`}>
          {currentProject ? currentProject.name : 'Navigation'}
        </div>
        <div className="flex flex-col gap-0.5">
          {visibleNavItems.map((item) => {
            const IconComponent = PRIMARY_ICON_MAP[item.icon];
            const useProjectIcon = item.path === '/' && item.exact && currentProject;
            return (
              <NavLink
                key={item.path}
                to={item.path}
              ...
              >
                {useProjectIcon
                  ? <ProjectFavicon domain={currentProject.domain} name={currentProject.name} size={18} />
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              ...
              <div className={`flex-1 min-w-0 whitespace-nowrap transition-opacity duration-200 ${collapsed ? 'opacity-0' : 'opacity-100'}`}>
                <div className="text-[13px] font-medium text-text-primary truncate">
                  {user?.name || user?.email}
                </div>
                <div className="text-[11px] text-text-faint">Free</div>
              </div>
              <span className={`text-[11px] font-medium text-text-muted border border-border-default rounded-full px-2.5 py-0.5 shrink-0 hover:bg-bg-surface-hover hover:text-text-secondary transition-opacity duration-200 ${collapsed ? 'opacity-0' : 'opacity-100'}`}>
                Upgrade
              </span>
            </button>
          </DropdownMenuTrigger>
          ...
          >
              ...
              <div className="flex items-center px-3 py-2">
                <span className="flex-1 text-[13px] font-medium text-text-secondary">Theme</span>
